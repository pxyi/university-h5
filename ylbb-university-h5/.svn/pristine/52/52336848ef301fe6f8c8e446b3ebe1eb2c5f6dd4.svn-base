function bubble() {
    ...
    var c = document.getElementById('bubble'),
      ...
      h = c.height = 31.2 * 100 * (document.documentElement.clientWidth / 750);

  
    var i, bubblesNumber = 7,
      objects = [],
      maxRadius = 35,
      maxYVelocity = 2;
    ...
    for (i = 0; i < bubblesNumber; i++) {
      objects.push(new Bubble(Math.random() * w, Math.random() * h, -randomInRange(.5, .7), randomInRange(5, maxRadius), randomInRange(7, 10), randomInRange(-40, 40), randomInRange(0, 360)));
    }
    ...
    window.addEventListener('resize', function() {
      ...
      h = world.physicalProperties.height = 31.2 * 100 * (document.documentElement.clientWidth / 750);
      $.globalCompositeOperation = 'lighter';
    });
  
    window.addEventListener('mousemove', function(e) {
      for (var i = 0; i < world.objects.length; i++) {
        if ((world.objects[i] instanceof Bubble) && (e.clientX > world.objects[i].x - world.objects[i].radius && e.clientX < world.objects[i].x + world.objects[i].radius)) {
    ...
    window.addEventListener('touchmove', function(e) {
      for (var i = 0; i < world.objects.length; i++) {
        if ((world.objects[i] instanceof Bubble) && (e.touches[0].clientX > world.objects[i].x - world.objects[i].radius && e.touches[0].clientX < world.objects[i].x + world.objects[i].radius)) {
          world.objects[i].pop(world);
        }
      }
    });
